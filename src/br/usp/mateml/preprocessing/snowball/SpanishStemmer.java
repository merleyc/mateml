...
public class SpanishStemmer extends SnowballStemmer {
	...
	/**
	 * Esse metodo stemmiza todas as palavras dos arquivos de um diretorio dado.
	 * @param dirArq diretorio contendo os arquivos que serao radicalizados.
	...
	private HashMap<String, Integer> stemmizarCorpusToHash(String dirArq) throws Throwable {
		...
		String[] lista_arq = (new File(dirArq)).list();

		for (int index = 0; index < lista_arq.length; index++) {
			String arquivoString = dirArq + "/" + lista_arq[index];
			File arquivo = new File(arquivoString);

			if (arquivo.isFile()) {

				if (!arquivo.exists()) {
					System.out.println("Nao encontrou o arquivo: " + arquivo.toString());
					return null;
				}

				if (!arquivo.getName().toLowerCase().endsWith("~")) {

					//reader = new InputStreamReader(new FileInputStream(arquivo));
					//reader = new BufferedReader(reader);
					String encoding = TestDetector.detectaEncodingImprimeArquivo(arquivoString);
					if (encoding != null) 
					...
					while ((character = reader.read()) != -1) {
						...
						if (Character.isWhitespace((char) ch)) {
							if (input.length() > 0) {
								...
								if (!stem.equals("")) {
									int freqPal = 1;
									if (hashStemFreq.containsKey(stem))
										freqPal = hashStemFreq.get(stem) + 1;

									hashStemFreq.put(stem, freqPal);
								}
	...
	/**
	 * Esse metodo retorna um stem de uma string dada.
	 * Nao realiza nenhum tratamento na string dada.
	 * @param st string a ser stemmizada / radicalizad.
